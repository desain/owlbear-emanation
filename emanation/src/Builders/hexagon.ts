...
export function buildHexagonGridEmanation(position: Vector2, numHexes: number, hexSize: number, absoluteItemSize: number, gridType: HexGridType) {
    ...
    return buildCurve()
        .points([
            ...points.map((point) => clockwiseAroundOrigin(point, utils.baseRotationDegrees)),
            ...points.map((point) => clockwiseAroundOrigin(point, utils.baseRotationDegrees + 60)),
            ...points.map((point) => clockwiseAroundOrigin(point, utils.baseRotationDegrees + 120)),
            ...points.map((point) => clockwiseAroundOrigin(point, utils.baseRotationDegrees + 180)),
            ...points.map((point) => clockwiseAroundOrigin(point, utils.baseRotationDegrees + 240)),
            ...points.map((point) => clockwiseAroundOrigin(point, utils.baseRotationDegrees + 300)),
        ])
        .position(position)
        .closed(true)
        .tension(0)
