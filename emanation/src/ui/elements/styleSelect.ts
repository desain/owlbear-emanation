...
import { EMANATION_STYLE_TYPES, EmanationStyleType } from '../../types/EmanationStyle';
import createFormControl from './formControl';
import { attrsToSpecifier, Specifier, specifierToHtml } from './specifier';
const EMANATION_STYLE = 'emanation-style';
...
export function createStyleSelect(specifier: Specifier | null, selectedOption: EmanationStyleType) {
    return createFormControl('Style', `
        <div
            class="mdc-select mdc-select--outlined ${EMANATION_STYLE}"
        ...
        >
            ...
            <div class="mdc-select__menu mdc-menu mdc-menu-surface mdc-menu-surface--fullwidth">
                <ul class="mdc-deprecated-list" role="listbox" aria-label="Style selector">
                    ${EMANATION_STYLE_TYPES.map((option) => selectItem(option, option === selectedOption)).join('')}
                </ul>
            </div>
        </div>
    `);
...
export function installStyleChangeHandler(handler: (style: EmanationStyleType, specifier: Specifier | null) => void) {
    document.querySelectorAll<HTMLElement>(`.${EMANATION_STYLE}`).forEach((element) => {
        const select = mdc.select.MDCSelect.attachTo(element);
        select.listen('MDCSelect:change', () => {
            handler(select.value as EmanationStyleType, attrsToSpecifier(element.dataset));
        });
    });
}