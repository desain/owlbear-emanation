uniform float vortexRadius; // Radius of the vortex effect in grid units
...
vec4 main(vec2 fragCoord) {
  ...
  // Fade out effect based on vortexRadius
  float falloff = smoothstep(vortexRadius + 1.0, vortexRadius - 1.0, d); // Smooth fade over 2 units
  if (falloff <= 0.0) {
  ...
  float tiling = 1.0; // Adjust tiling if needed, maybe based on vortexRadius
