...
function buildDrawable(
    ...
    if (grid.measurement === 'CHEBYSHEV' && grid.type === 'SQUARE') {
        return buildChebyshevSquareAura(position, numUnits, grid.dpi, absoluteItemSize);
    } else if (grid.measurement === 'CHEBYSHEV' && (grid.type === 'HEX_HORIZONTAL' || grid.type === 'HEX_VERTICAL')) {
        if (sceneMetadata.gridMode) {
            return buildHexagonGridAura(position, Math.round(numUnits), grid.dpi, absoluteItemSize, grid.type);
        } else {
            // TODO normal hexagon building code, this is hacky
            const aura = buildHexagonGridAura(position, 0, grid.dpi, 0, grid.type);
        ...
        if (sceneMetadata.gridMode) {
            return octantToCurve(buildManhattanSquareOctant(numUnits), grid.dpi, absoluteItemSize, position);
        } else {
            return buildManhattanPrecise(position, numUnits, grid.dpi, absoluteItemSize);
        }
...
export function buildSimpleAura(
    ...
    const drawable: SimpleAuraDrawable = buildDrawable(sceneMetadata, grid, position, numUnits, absoluteItemSize);
