...
export async function getPlayerMetadata(apiMetadata?: Metadata): Promise<PlayerMetadata> {
    const metadata = apiMetadata ?? await OBR.player.getMetadata();
    return metadata[METADATA_KEY] as PlayerMetadata | undefined ?? defaultPlayerMetadata();
}
