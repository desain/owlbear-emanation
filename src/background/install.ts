...
async function rebuildAuras(
    fixer: LocalItemFixer,
    grid: GridParsed,
    ids?: string[],
) {
...
function installSceneMetadataWatcher(
    withLock: (runnable: () => Promise<void>) => Promise<void>,
    fixer: LocalItemFixer,
    getGrid: () => GridParsed,
) {
    ...
    return OBR.scene.onMetadataChange(async (metadata) => {
        return await withLock(async () => {
            ...
            if (
                ...
                (oldMetadata === null ||
                    sceneMetadataChanged(newMetadata, oldMetadata))
            ) {
